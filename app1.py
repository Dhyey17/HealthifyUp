...
def heart():
    if request.method == 'GET':
        ...
        new = np.array([[Age, gender, cp, trestbps, chol, fbs, restecg, thalach, ds['exang'].mean(),
                         ds['oldpeak'].mean(), ds['slope'].mean(), ds['ca'].mean(), ds['thal'].mean()]])
...
def liver():
    if request.method == 'GET':
        ...
        new = np.array([[Age, Total_Bilirubin, df1['Direct_Bilirubin'].mean(), Alkaline_Phosphotase,
                         Alamine_Aminotransferase, Aspartate_Aminotransferase, Total_Protiens, Albumin,
                         df1['Albumin_and_Globulin_Ratio'].mean(), gender]])
        y_pred = model.predict(new)
...
def backpain():
    if request.method == "GET":
        ...
        model.fit(X, y)
        new = np.array([[pelvic_incidence, pelvic_tilt, lumbar_lordosis_angle, sacral_slope, pelvic_radius,
                         restecgdegree_spondylolisthesis, pelvic_slope, Direct_tilt, thoracic_slope, cervical_tilt,
...
def tb():
    if request.method == "GET":
        ...
        new = np.array(
            [[fatigue, weight_loss, loss_of_appetite, mild_fever, chills, sweating, mucoid_sputum, cough, chest_pain]])
        y_pred = model.predict(new)
        return f'{y_pred}'
